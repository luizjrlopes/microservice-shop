...
| Java JDK | 17 | Build do `order-service` com Maven. |
| Maven | 3.9 | Execução de testes/build Spring Boot. |
| Python | 3.11 | Execução standalone do `scheduler-agent` e notebooks de experimentos. |
| Node.js + npm | 18 / 9 | Testes BDD (`tests/bdd`). |
| Make (opcional) | 4+ | Automação de scripts locais. |
...
- Os notebooks de IA/LLM devem residir em `./notebooks/` e seguir a convenção descrita em [`docs/experiments-notebooks.md`](./experiments-notebooks.md).
